...
{
    printk("pashahod_chdev: Device ioctl with cmd = %u\n", cmd);
    return 0;
}
...
static int __init pashahod_chdev_init(void)
{
    ...
    err = cdev_add(&pashahod_chdev_data.cdev, MKDEV(dev_major, 0), 0);
    if (err) {
    ...
    device_create(pashahod_chdev_class, NULL, MKDEV(dev_major, 0), NULL, "pashahod_chdev-%d", 0);
...
MODULE_DESCRIPTION("Pashahod chardev (c)");
MODULE_LICENSE("GPL");
